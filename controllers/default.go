...
func init() {
   ...
   orm.RegisterModelWithPrefix("blog_", new(m.User),new(m.Article))
    ...
    orm.RegisterDataBase("default", "mysql", "badassblog:gotobadass1A@/zadmin_badassblog?charset=utf8", 30)
...
func ListAtricleByOffsetAndLimit(i int,c int,isCount bool) (*[] *m.Article,int64) {
  ...
  if isCount{
   num,_ :=o.QueryTable(new(m.Article)).Filter("Id__gt",0).All(&articles)
     return &articles ,num
  }else{
